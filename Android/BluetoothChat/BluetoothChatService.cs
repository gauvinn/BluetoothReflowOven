...
namespace BluetoothChat
{
	...
	class BluetoothChatService
	{
		...
		private void SetState(int state)
		{
			...
			_handler.ObtainMessage((int)BluetoothChat.MESSAGE_COMMANDS.MESSAGE_STATE_CHANGE, state, -1).SendToTarget();
		}
		...
		{	
			...
			if(acceptThread == null) {
				acceptThread = new AcceptThread(this, _uuid);
				acceptThread.Start();
			}
		...
		public void Connect(BluetoothDevice device)
		{
			...
			connectThread = new ConnectThread(device, this, _uuid);
			connectThread.Start();
		...
		public void Connected(BluetoothSocket socket, BluetoothDevice device)
		{
			...
			var msg = _handler.ObtainMessage((int)BluetoothChat.MESSAGE_COMMANDS.MESSAGE_DEVICE_NAME);
		...
		private void ConnectionFailed()
		{
			...
			var msg = _handler.ObtainMessage((int)BluetoothChat.MESSAGE_COMMANDS.MESSAGE_TOAST);
			Bundle bundle = new Bundle();
		...
		private class AcceptThread : Thread
		{
			...
			public AcceptThread(BluetoothChatService service, UUID uuid)
			{
				_service = service;
				...
				try {
					tmp = _service._adapter.ListenUsingRfcommWithServiceRecord(NAME, uuid);
		...
		protected class ConnectThread : Thread
		{
			...
			public ConnectThread(BluetoothDevice device, BluetoothChatService service, UUID uuid)
			{
				mmDevice = device;
				_service = service;
		...
		private class ConnectedThread : Thread
		{
			...
			public override void Run()
			{
				Log.Info(TAG, "***ConnectedThread Running***");
				...
				while(true)
                {
                    if(mmInStream.IsDataAvailable())
                    {
                        ...
                        try
                        {
                            ...
                            _service._handler.ObtainMessage((int)BluetoothChat.MESSAGE_COMMANDS.MESSAGE_READ, bytes, -1, buffer).SendToTarget();
                        }
                        catch(Java.IO.IOException e)
                        {
			...
			public void Write(byte[] buffer)
			{
				try
                {
					...
					_service._handler.ObtainMessage((int)BluetoothChat.MESSAGE_COMMANDS.MESSAGE_WRITE, -1, -1, buffer).SendToTarget();
				}
                catch(Java.IO.IOException e)
                {
